...
config=new ConfigIniParser;var mqtt,reconnectTimeout,reconnect,host,port,clientId,weatherTopic,indicatorTopic,radioTopic,kodiTopic,commandTopic,kodiPlaybackState=0,latitude,longitude,locale=Intl.DateTimeFormat().resolvedOptions().locale,timezoneLong=Intl.DateTimeFormat().resolvedOptions().timeZone,i18n,updateLighting;
...
function onConnect(){console.log("Connected to broker "+host+":"+port);console.log("Subscribing to weather status at "+weatherTopic+"#");mqtt.subscribe(weatherTopic+"#");console.log("Subscribing to radio status at "+radioTopic+"#");mqtt.subscribe(radioTopic+"#");console.log("Subscribing to studio status at "+indicatorTopic+"#");mqtt.subscribe(indicatorTopic+"#");console.log("Subscribing to Kodi status at "+kodiTopic+"#");mqtt.subscribe(kodiTopic+"#");console.log("Subscribing to device command at "+
commandTopic+"#");mqtt.subscribe(commandTopic+"#")}function onFailure(a){console.log("Connection to "+host+":"+port+" failed");setTimeout(MQTTconnect,reconnectTimeout)}
...
function MQTTconnect(){console.log("Connecting to "+host+":"+port);mqtt=new Paho.MQTT.Client(host,port,clientId);var a={timeout:3,reconnect:reconnect,onSuccess:onConnect,onFailure:onFailure};mqtt.onConnectionLost=onConnectionLost;mqtt.onMessageArrived=onMessageArrived;mqtt.connect(a)}var configfile=location.hostname?"config/"+location.hostname+".cfg":"config/unknown_host.cfg";console.log("Read configuration from "+configfile);fetch(configfile,{cache:"reload",mode:"no-cors"}).then(function(a){return a.text()}).then(function(a){return init1(a)});
function init1(a){console.log("Init stage 1: Configuration");config.parse(a);reconnectTimeout=config.getNumber(mySection,"reconnect_timeout");reconnect=config.getBoolean(mySection,"reconnect");host=config.get(mqttSection,"host");port=config.getNumber(mqttSection,"websockets_port");clientId=config.get(mySection,"client_id");weatherTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"weather_topic");indicatorTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"indicator_topic");
radioTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"radio_topic");kodiTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"kodi_topic");commandTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"client_topic")+"command/";latitude=config.getNumber(mySection,"latitude");longitude=config.getNumber(mySection,"longitude");timezoneLong=config.get(mySection,"timezone");locale=config.get(mySection,"locale");document.title=config.get(mySection,"title");setElement("title",
config.get(mySection,"title"));fetch("config/lang-"+locale+".json",{cache:"reload",mode:"no-cors"}).then(function(a){return a.json()}).then(function(a){return init2(a)})}
