...
config=new ConfigIniParser;var mqtt,reconnectTimeout,reconnect,host,port,clientId,astronomyTopic,commandTopic,latitude,longitude,locale=Intl.DateTimeFormat().resolvedOptions().locale,timezoneLong=Intl.DateTimeFormat().resolvedOptions().timeZone,i18n;function date2HHmm(a){return a.toLocaleString(locale,{hour:"2-digit",minute:"2-digit",timeZone:timezoneLong})}
...
function onConnect(){console.log("Connected to broker "+host+":"+port);console.log("Subscribing to astronomy status at "+astronomyTopic+"#");mqtt.subscribe(astronomyTopic+"#");console.log("Subscribing to device command at "+commandTopic+"#");mqtt.subscribe(commandTopic+"#")}function onFailure(a){console.log("Connection to "+host+":"+port+" failed");setTimeout(MQTTconnect,reconnectTimeout)}
...
function init1(a){console.log("Init stage 1: Configuration");config.parse(a);reconnectTimeout=config.getNumber(mySection,"reconnect_timeout");reconnect=config.getBoolean(mySection,"reconnect");host=config.get(mqttSection,"host");port=config.getNumber(mqttSection,"websockets_port");clientId=config.get(mySection,"client_id");astronomyTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"astronomy_topic");commandTopic=config.get(mqttSection,"base_topic")+config.get(mySection,"client_topic")+
"command/";latitude=config.getNumber(mySection,"latitude");longitude=config.getNumber(mySection,"longitude");timezoneLong=config.get(mySection,"timezone");locale=config.get(mySection,"locale");document.title=config.get(mySection,"title");fetch("config/lang-"+locale+".json",{cache:"reload",mode:"no-cors"}).then(function(a){return a.json()}).then(function(a){return init2(a)})}
